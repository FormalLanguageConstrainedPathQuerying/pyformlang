...
class TestDeterministicFiniteAutomaton:
    ...
    def test_can_create(self) -> None:
        ...
        assert dfa is not None
        dfa = DeterministicFiniteAutomaton(
            start_state=state1, final_states={state0, state1}
        )
        assert dfa is not None
    ...
    def test_add_remove_start_final(self) -> None:
    ...
    def test_big_minimize(self) -> None:
        dfa = DeterministicFiniteAutomaton()
        size = 1000
        symb = Symbol("s")
    ...
    def test_big_minimize_reduce(self) -> None:
        dfa = DeterministicFiniteAutomaton()
        symb_0 = Symbol("0")
    ...
    def test_minimize_repetition(self) -> None:
        dfa = DeterministicFiniteAutomaton()
        symb_a = Symbol("a")
        symb_b = Symbol("b")
        symb_star = Symbol("star")
    ...
    def test_not_cyclic2(self) -> None:
        dfa = DeterministicFiniteAutomaton()
        state0 = State(0)
        state1 = State(1)
        symb_a = Symbol("a")
    ...
    def test_dfa_generating_no_words(self) -> None:
...
def get_dfa_example_without_accepted_words():
    ...
    symbol_b = Symbol("b")
    dfa.add_transitions(
        [(states[0], symbol_a, states[1]), (states[2], symbol_b, states[3])]
    )
    dfa.add_start_state(states[0])
    dfa.add_final_state(states[3])
    return dfa
...
def perform_tests_example0(enfa: EpsilonNFA) -> None:
